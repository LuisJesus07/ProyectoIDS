<?php 
		try{
			...
			$query = "SELECT * FROM administrador WHERE correoE= :correoE AND password= :password";//Se hace la consulta SQL
			...
			if ($numero_registro!=0){//Sirve para saber si hay registros en la BD
				...
				$_SESSION['administrador'] =$_POST['correoE']; //Le pasamos el nombre del usuario ala sesion
				header("location:menuJuego.php");//Redirigimos la pagina si es que el usuario existe en la BD al inicio

			}else{
				header("location:loginAdminError.php");//Si no existe lo redirigimos al login de nuevo
			}
